...
showRow' ps = beside (f ( g ps))
 ...
 -- EXPERIMENTATION: trying to show a list of list of integers into a grid like tictactoe. --
...
showRowInt :: [Int] -> [String]
showRowInt = beside . interleave bar . map showInt
               where
                  beside = foldr1 (zipWith (++))
                  bar = replicate 3 "|"

putGridInt :: [[Int]] -> IO ()
putGridInt = putStrLn . unlines . concat . interleave bar . map showRowInt 
            where 
